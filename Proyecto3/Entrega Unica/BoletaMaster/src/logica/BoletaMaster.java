...
public class BoletaMaster implements IPuertoTiquetes, IPuertoUsuarios  {
	...
	public List<Tiquete> boletasDeCliente(Cliente c) {
	    return c.getTiquetes();
	}
    public Tiquete prepararImpresionBoleta(Cliente cliente, int idTiquete) {
        Tiquete tiq = null;
        for (Tiquete t : tiquetes) { 
            if (t.getIdTiquete() == idTiquete) {
                tiq = t;
        ...
        if (tiq == null)
            throw new IllegalArgumentException("No existe un tiquete con ese ID");

        if (!tiq.getComprador().equals(cliente))
            throw new IllegalArgumentException("Ese tiquete no pertenece al cliente");

        if (tiq.estaImpreso())
            throw new IllegalStateException("El tiquete ya fue impreso y no se puede reimprimir");
        tiq.marcarComoImpreso();
        return tiq;
    }
